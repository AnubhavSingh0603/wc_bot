...
class EmojiStatsCog(commands.Cog):
    ...
    @app_commands.command(name="emoji", description="Top emoji stats (server-wide, or filter by user).")
    @app_commands.describe(
        n="How many emojis to show in top/bottom lists (default 10).",
        user="Optional: show top emoji used by this user instead of the whole server.",
    )
    async def emoji(
        self,
        ...
        gid = int(guild.id)
        target_uid = int(user.id) if user is not None else None
        scope_label = (
            f"{user.display_name}" if user is not None else "Server"
        )
        ...
        # Server (or user) totals from DB (custom emojis)
        ...
        # Page 1: Server top (or user top)
        e1 = base_embed(
            f"Top Emoji Stats — {scope_label}",
            (
                f"Top **{n}** **server custom emojis** "
                + ("used by this user." if target_uid is not None else "(includes 0-count emojis).")
            ),
        )
        e1.add_field(name=f"Top {n}", value="\n".join(fmt(p) for p in top) or "—", inline=False)

        # Page 2: Server bottom (or user-bottom = least-used by user)
        e2 = base_embed(
            f"Least Used Emoji — {scope_label}",
            (
                f"Bottom **{n}** **server custom emojis** "
                + ("used least by this user." if target_uid is not None else "(includes unused / 0-count emojis).")
            ),
        )
        ...
        # Page 3: Unicode top
        e3 = base_embed(
            f"Top Unicode Emoji — {scope_label}",
            (
                f"Top **{n}** unicode emoji "
                + ("used by this user (messages + reactions)."
                   if target_uid is not None
                   else "in this server (messages + reactions).")
            ),
        )
