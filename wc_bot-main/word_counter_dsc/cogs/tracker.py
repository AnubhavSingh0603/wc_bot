...
class TrackerCog(commands.Cog):
    ...
    def clear_stopword_cache(self, guild_id: int | None = None) -> None:
        """Invalidate cached stopwords after /stopword add/remove changes.

        This makes new stopwords effective immediately while still keeping the
        normal low-DB-use TTL cache during regular message tracking.
        """
        if guild_id is None:
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Canonical keyword counts include configured roots embedded in transformed
        # words, e.g. fuck/fucks/fucking/absofuckinglutely -> fuck and
        # tit/tits/titties/mustitties -> tit. This is keyword-driven and does
        # not make arbitrary substrings visible unless the root is configured.
        ...
        for kw, c in keyword_counts.items():
            ...
            await self.bot.dbx.execute(
                ...
                DO UPDATE SET count = word_counts.count + excluded.count,
                              updated_at = excluded.updated_at
                """,
                (gid, cid, uid, str(kw), int(extra), now),
            )
